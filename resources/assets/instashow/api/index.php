...
$path = App\asset_path('instashow/api/config.json');

$remove_string = get_template_directory_uri();
$remove_string = str_replace('resources', 'dist/instashow/api', $remove_string);


$path = str_replace($remove_string, "", $path);

main(__DIR__ . $path, __DIR__ . '/storage');
...
function top_search($entity, $type) {
    $search_res = client_request('get', '/web/search/topsearch/', array(
        'query' => array(
            'context' => 'blended',
            'query' => $entity,
            'count' => 1
...
function serve_user($username) {
    ...
    if (!$raw_data) {
        $user_data = top_search($username, 'user');
        ...
        if ($user_data['is_private']) {
            ...
            $query_follows = query_client_request(array(), array('id' => $user_data['id'],'first' => 0), 'd04b0a864b4b54837c0d870b0e77e076');

            $query_follows_data = json_decode($query_follows['body'], true);
            $raw_data['follows_count'] = $query_follows_data['data']['user']['edge_follow']['count'];

            $query_media = query_client_request(array(), array('id' => $user_data['id'], 'first' => 0), 'f2405b236d85e8296cf30347c9f08c2a');
            
            $query_media_data = json_decode($query_media['body'], true);
            $raw_data['media_count'] = $query_media_data['data']['user']['edge_owner_to_timeline_media']['count'];
    ...
    if ($raw_data) {
        $formatted_data = array(
            ...
            'counts' => array(
                'media' => $raw_data['media_count'],
                'followed_by' => $raw_data['follower_count'],
                'follows' => $raw_data['follows_count']
            )
        );
    ...
    if ($storage_expired_or_empty) {
        ...
        if (empty($user_data)) {
            ...
            if ($user_data['is_private']) {
                ...
                $formatted_data = user_media_recent_query_recursive($cache_key, array(), $user_data, array('id' => $user_data['id'], 'first' => 50), $limit, 0, array());
            }
        }
    }
...
function serve_location_media_recent($location_id) {
    ...
    if ($storage_expired_or_empty) {
        $formatted_data = location_media_recent_query_recursive($cache_key, array(), array('id' => $location_id, 'first' => 50), $limit, 0, array());
    }
...
function query_client_request($page_data, $variables, $query_hash) {
    ...
    return client_request('get', '/graphql/query/', array(
        ...
        'headers' => array(
            'X-Csrftoken' => !empty($page_data) ? $page_data['config']['csrf_token'] : null,
